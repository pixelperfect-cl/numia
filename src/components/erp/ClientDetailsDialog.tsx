...
import { getSubscriptions, deleteSubscription } from '@/lib/supabase/database';
...
export function ClientDetailsDialog({ client, open, onOpenChange, onEditClient }: ClientDetailsDialogProps) {
    ...
    // Prevent rendering if not open, but keep component structure valid
            ...
            <Dialog open={open} onOpenChange={onOpenChange}>
                <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
                    ...
                    {client ? (
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex items-center gap-3 text-sm text-muted-foreground mt-1">
                                                            ...
                                                            {(() => {
                                                                ...
                                                                const date = new Date(sub.nextBillingDate + 'T00:00:00');
                                                                if (!isValid(date)) return null;

                                                                return (
                                                                    <span className="flex items-center gap-1">
                                                                        <Calendar className="h-3 w-3" />
                                                                        Prox: {format(date, 'dd/MM/yyyy')}
                                                                    </span>
                                                                );
                                                            })()}
                                                        </div>
                        ...
                        <div className="py-8 text-center text-muted-foreground">Cargando información del cliente...</div>
                    )}
                </DialogContent>
            </Dialog>
